...
class F1RacePredictor:
    ...
    # Data collection
    # ------------------------------------------------------------------
    def collect_session_laps(self, year, gp_name, session_type):
        try:
            ...
            return pd.DataFrame(columns=['Driver', 'LapTime', 'Stint'])

    def extract_weekend_features(self, gp_name):
        """Separate qualifying-style and race-style runs from practice data."""
        all_laps = []
        year = self.year if self.year <= datetime.now().year else datetime.now().year
        for session in ['FP1', 'FP2', 'FP3']:
            laps = self.collect_session_laps(year, gp_name, session)
            if not laps.empty:
        ...
        # Classify stints by their length
        if 'Stint' in laps.columns:
            stint_len = laps.groupby(['Driver', 'Stint'])['LapTime'].transform('count')
        else:
            stint_len = laps.groupby('Driver')['LapTime'].transform('count')
        laps['StintLaps'] = stint_len

        quali_laps = laps[laps['StintLaps'] <= 3]
        race_laps = laps[laps['StintLaps'] >= 5]
        if quali_laps.empty:
            quali_laps = laps
        if race_laps.empty:
            race_laps = laps

        quali_runs = quali_laps.groupby('Driver')['LapTime'].min().reset_index()
        quali_runs.rename(columns={'LapTime': 'BestLap'}, inplace=True)

        race_runs = race_laps.groupby('Driver')['LapTime'].mean().reset_index()

...
class F1RacePredictor:
    ...
    def prepare_features_for_ml(
        ...
        """Combine all aggregates into a single feature table with team metrics."""
        if quali_runs.empty:
            return pd.DataFrame()

        features = quali_runs.merge(driver_team, on='Driver', how='left')
        features = features.merge(race_runs, on='Driver', how='left')
        features = features.merge(stats, on='Driver', how='left')

        team_feats = team_quali.merge(team_race, on='Team', how='left')
        team_feats = team_feats.merge(team_stats, on='Team', how='left')
        features = features.merge(team_feats, on='Team', how='left')
        ...
        features['TeamBestLap'].fillna(features['BestLap'], inplace=True)
        ...
        team_fastest = features['TeamBestLap'].min()
        features['team_gap'] = features['TeamBestLap'] - team_fastest
        return features
    ...
    def predict_qualifying(self, features):
        if features.empty:
            return pd.DataFrame()
        # Penalize inconsistent laps and slower cars
        features['qual_score'] = (
            features['BestLap']
            + 0.05 * features['LapStd']
            + 0.1 * features['team_gap']
        )
    ...
    def predict_race(self, features, quali_predictions):
        ...
        features['team_rank'] = features['TeamBestLap'].rank()

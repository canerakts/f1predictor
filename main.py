...
class F1RacePredictor:
    ...
    def collect_practice_data(self, year, gp_name, session_type):
        try:
            ...
            return pd.DataFrame(columns=['Driver', 'Team', 'LapTime'])
    ...
    def extract_weekend_features(self, gp_name):
        ...
        team_features = team_quali.merge(team_race, on='Team', how='outer')
        team_features = team_features.merge(team_stats, on='Team', how='outer')

        return quali_runs, race_runs, stats, team_features

    # ------------------------------------------------------------------
    # Feature preparation
    # ------------------------------------------------------------------
    def prepare_features_for_ml(self, quali_runs, race_runs, stats, team_features):
        ...
        features = features.merge(team_features, on='Team', how='left')
        ...
        features['TeamAvgLap'].fillna(features['AvgLap'], inplace=True)
        features['TeamMedianLap'].fillna(features['MedianLap'], inplace=True)
    ...
    def predict_qualifying(self, features):
        if features.empty:
            return pd.DataFrame()
        # Blend driver pace with team performance and consistency
        features['qual_score'] = (
            features['BestLap'] * 0.7
            + features['TeamBestLap'] * 0.3
            + 0.05 * features['LapStd']
            + 0.05 * features['TeamLapStd']
        )
        ...
        team_factor = team_factor.clip(lower=0.5)
    ...
    def simulate_race(self, race_predictions, num_sim=1000):
        ...
        max_std = max(lap_std_sec.max(), team_std_sec.max(), 1)
        max_laps = max(num_laps.max(), team_laps.max(), 1)

        std_factor = (lap_std_sec + team_std_sec) / max_std
        laps_factor = 2 - ((num_laps + team_laps) / max_laps)
    ...
    def run_prediction(self, gp_name):
        quali_runs, race_runs, stats, team_feats = self.extract_weekend_features(gp_name)
        features = self.prepare_features_for_ml(quali_runs, race_runs, stats, team_feats)

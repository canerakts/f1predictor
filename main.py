...
class F1RacePredictor:
    ...
    def simulate_race(self, race_predictions, num_sim=1000):
        ...
        lap_std = race_predictions.get('LapStd', pd.Series([0]*len(base_pos)))
        ...
        lap_std_ratio = lap_std_sec / lap_std_sec.max() if lap_std_sec.max() != 0 else 0
        lap_count_ratio = num_laps / num_laps.max() if num_laps.max() != 0 else 0

        noise_scale = 1.5 + lap_std_ratio + (1 - lap_count_ratio)

        for _ in range(num_sim):
            noise = np.random.normal(0, noise_scale, size=len(base_pos))
            pos = np.clip(base_pos + noise, 1, 20)
            order = pd.Series(pos).rank(method='first').astype(int)
            results.append(order.values)

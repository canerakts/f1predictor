...
class F1RacePredictor:
    ...
    def extract_weekend_features(self, gp_name):
        ...
        if {'Sector1Time', 'Sector2Time', 'Sector3Time'}.issubset(laps.columns):
            team_sectors = (
                laps.groupby('Team')[['Sector1Time', 'Sector2Time', 'Sector3Time']]
                .min()
            )
            team_ideal = team_sectors.sum(axis=1).reset_index(name='TeamIdealLap')
        else:  # fallback if sector times unavailable
            team_ideal = (
                laps.groupby('Team')['LapTime']
                .min()
                .reset_index(name='TeamIdealLap')
            )
        ...
        teams = team_quali.merge(team_ideal, on='Team', how='outer').merge(team_stats, on='Team', how='outer')
    ...
    def prepare_features_for_ml(self, quali_runs, race_runs, stats, team_data):
        ...
        team_cols = ['TeamBestLap', 'TeamIdealLap', 'TeamMedianLap', 'TeamLapStd', 'TeamNumLaps']
        for c in team_cols:
            if c in features:
        ...
        features['team_gap'] = features['TeamIdealLap'] - features['BestLap']
        return features
    ...
    def predict_race(self, features, quali_predictions):
        ...
        features['team_pace_rank'] = features['TeamIdealLap'].rank()
